...
filename = "virtual_karst_conduit_13"
...
U_upper = 1e-4 
U_lower = 5e-5
U = U_upper # use the incision at the upper boundary as a reference
E_limestone = 3.0e-5 #5e-5 # limestone surface chemical denudation rate (m/yr)
E_weathered_basement = 5.0e-5 # weathered basement surface chemical denudation rate (m/yr)
K_sp_limestone = 1e-6 # streampower incision (yr^...)
K_sp_basement = 1e-6 # streampower incision (yr^...)
...
b_limestone = 200 # limestone unit thickness (m)
...
ie_frac = 0.01 # fraction of r_tot that becomes overland flow on limestone. ie_frac on basement=1. 
...
# effectively decrease the baselevel fall rate of the upper boundary, staying in the reference frame of 
# the upper boundary
dz_ad = np.zeros(mg.size("node"))
dz_ad[mg.core_nodes] = U * dt
dz_ad[bottom_nodes] = U_lower * dt 
...
# set an initial relief of limestone thickness at upper boundary
# update topography
dz_ad_0 = np.zeros(mg.size("node"))
dz_ad_0[mg.core_nodes] = b_limestone
dz_ad_0[bottom_nodes] = b_limestone
z += dz_ad_0
lith.dz_advection = dz_ad_0
lith.run_one_step()
zk[:] = z-lith.z_top[0,:]
...
ld2 = TaylorNonLinearDiffuser(mg, linear_diffusivity=D_ld, slope_crit=Sc, nterms=n_terms, dynamic_dt=True, courant_factor=0.1) # check dt_max, but it's usually large
...
params = {'U':U, 'U_lower':U_lower, 'K_limestone':K_sp_limestone, 'K_basement':K_sp_basement, 
